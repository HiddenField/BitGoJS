...
export class Eth extends BaseCoin {
  ...
  protected validateBatchData(prebuildRecipients: EthRecipient[], originalRecipients: EthRecipient[]): void {
    ...
    if (typeof(batchData) !== 'string' || !batchData.startsWith('0x')) {
      throw new Error('batch data must be a hex string');
    }

    const parsedSendAmount = this.safeStringNumber(sendAmount);
    if (!parsedSendAmount) {
    ...
    for (let i = 0; i < addresses.length; i++) {
      ...
      const amount = this.safeStringNumber(amounts[i].toString());
      if (!amount) {
    ...
    if (!totalAmount.eq(parsedSendAmount)) {
